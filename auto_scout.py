...
# ─────────────────────────────────────────────
# WPILOG Writer
# Uses double[] entries — no struct schema required.
# Per spec: double[] payload is raw IEEE-754 doubles with NO length prefix;
# array length is inferred from payload size / 8.
# ─────────────────────────────────────────────
class WPILogWriter:
    ...
    def _write_file_header(self):
        # Magic(6) + version major(1) + version minor(1) + extra header len(4) = 12 bytes
    ...
    def start_entry(self, name, type_str):
        """Register a new entry and return its integer ID."""
        ...
        payload = (
            struct.pack("<BI", 0, eid) +           # ctrl type=Start(0), entry ID
    ...
    def write_pose2d(self, eid, ts, x_m, y_m, rot_rad):
        """
        Write a Pose2d as double[] [x_meters, y_meters, rotation_radians].
        NO length prefix — spec says double[] length is inferred from payload size.
        """
        data = struct.pack("<ddd", float(x_m), float(y_m), float(rot_rad))
        self._write_record(eid, ts, data)
...
def process_match(
    ...
    print("[INFO] Video: {:.1f} fps, {} frames, {:.1f}s".format(video_fps, total_frames, duration_sec))
    ...
    # One double[] entry per robot for pose [x_m, y_m, heading_rad]
    # One boolean entry per robot for visibility
    robot_pose_eids = []
    robot_vis_eids  = []
    for i in range(4):
        robot_pose_eids.append(log.start_entry("Robot{}/Pose".format(i), "double[]"))
        robot_vis_eids.append( log.start_entry("Robot{}/Visible".format(i), "boolean"))
    ...
    while True:
        ...
        # write WPILOG — double[] [x_m, y_m, heading_rad], no length prefix
        for i, pose in enumerate(poses):
            log.write_pose2d(robot_pose_eids[i], timestamp_us,
                             pose.x_m, pose.y_m, pose.heading)
            log.write_boolean(robot_vis_eids[i], timestamp_us, pose.visible)
        ...
        if debug and debug_dir:
            ...
            if field_corners is not None:
                cv2.polylines(debug_frame, [field_corners.astype(int)], True, (255, 255, 0), 2)
            for i, pose in enumerate(poses):
                if pose.visible and H is not None:
                    H_inv = np.linalg.inv(H)
                    denom = H_inv[2,0]*pose.x_in + H_inv[2,1]*pose.y_in + H_inv[2,2]
                    if denom != 0:
                        px_d = int((H_inv[0,0]*pose.x_in + H_inv[0,1]*pose.y_in + H_inv[0,2]) / denom)
                        py_d = int((H_inv[1,0]*pose.x_in + H_inv[1,1]*pose.y_in + H_inv[1,2]) / denom)
                        cv2.circle(debug_frame, (px_d, py_d), 20, colors[i], 3)
                        cv2.putText(debug_frame, "R{}".format(i), (px_d - 10, py_d - 25),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, colors[i], 2)
                        dx = int(30 * math.cos(pose.heading))
                        dy = int(-30 * math.sin(pose.heading))
                        cv2.arrowedLine(debug_frame, (px_d, py_d), (px_d + dx, py_d + dy), colors[i], 2)
            cv2.putText(debug_frame, "t={:.2f}s".format(match_time_s),
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.imwrite(os.path.join(debug_dir, "frame_{:06d}.jpg".format(processed)), debug_frame)
...
|  5. Drag Robot0/Pose into the Poses section                      |
|  6. Click the icon to the LEFT of the field name in the sidebar  |
|     Set format: "Pose2d"  |  Units: Meters + Radians             |
|  7. Repeat for Robot1/Pose, Robot2/Pose, Robot3/Pose             |
|  8. Press play to watch the match replay!                        |
|                                                                  |
...
def download_video(url, output_dir):
    import subprocess
    out_path = os.path.join(output_dir, "match_video.mp4")
    print("[INFO] Downloading: {}".format(url))
    result = subprocess.run(
        ["yt-dlp", "-f", "best[ext=mp4]", "-o", out_path, url],
        check=True
    )
    return out_path
...
def main():
    ...
    parser.add_argument("url", nargs="?", help="YouTube URL of the match")
